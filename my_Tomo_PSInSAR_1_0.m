%利用Tomo-PSInSAR实现南京明城墙仪凤门地区层析成像(共26景，TerraSAR），预处理使用了GAMMA软件，具体过程参考GAMMA_script.txt.
...
file_name_date=baseline_file(:,1);
...
for row=1:baseline_number
    mean_baseline(row,1)=baseline_file(find(baseline_file(:,1)==file_name_date(row)),2);
end
%时间基线
...
b_k = temperature.';
%% SLC文件读取
gen_SPSC_PPSC_index_file(path,file_name_date,master,img_height,img_width);
PPSC_index = importdata([path,'PPSC_index.txt']);
SPSC_index = importdata([path,'SPSC_index.txt']);
path = workdir;%工作路径的确定
In = single(zeros(img_height,img_width));   %SLC数据
PS_slc = single(zeros(baseline_number,size(PPSC_index,1)));
for i = 1:baseline_number
    if file_name_date(i)==master
        fid = fopen([path,file_name_date(i),'.slc']);
    else
        fid = fopen([path,file_name_date(i),'.rslc']);
    end
    In = fread(fid,[img_height,img_width],'*short',2,'ieee-be').';
    frewind(fid);
    fread(fid,1,'*short');
    In = In + 1i*fread(fid,[img_height,img_width],'*short',2,'ieee-be').';
    fclose(fid);
    PS_slc(i,:) = In(PPSC_index);
end
clear In;
...
%     In(:,:,i) =In(:,:,i) + fread(fid2,[img_height,img_width],'int16=>single')*1i;
% end
% fclose(fid2);
% fclose(fid1);
...
PS_flat = single(zeros(baseline_number,size(PPSC_index,1)));
Int = single(zeros(img_height,img_width));
Diff = single(zeros(img_height,img_width));
for i = 1:baseline_number
    if file_name_date(i)==master%单独处理一下主影像的问题
        continue;
    else
        fid = fopen([path,file_name_date(i),'.int']);%注意这里的命名问题
        Int = fread(fid,[img_height,img_width],'*single',4,'ieee-be').';
        frewind(fid);
        fread(fid,1,'*single');
        Int = Int + 1i*fread(fid,[img_height,img_width],'*single',4,'ieee-be').';
        fclose(fid);
        fid = fopen([path,file_name_date(i),'.diff']);
        Diff = fread(fid,[img_height,img_width],'*single',4,'ieee-be').';
        frewind(fid);
        fread(fid,1,'*single');
        Diff = Diff + 1i*fread(fid,[img_height,img_width],'*single',4,'ieee-be').';
        fclose(fid);
        Int = angle(Int.*conj(Diff));
        PS_flat(i,:) = Int(PPSC_index);
    end
end
clear Int;
clear Diff;
PS_slc = PS_slc.*exp(1i*PS_flat);
%% 振幅离差指数阈值法筛选PS点
abs_In = abs(In);
abs_In_2 = abs_In.^2;
...
M = exp(-1i*2*pi*(S+V+K));
for i = 1:n_e
    y = squeeze(PS_slc(PPSC_index(e(i,2))).*conj(PS_slc(PPSC_index(e(i,1)))));
    norm_y = norm(y,2);
...
%         for n = 1:size(d_v,2)
%             for k = 1:size(d_k,2)
%                 a = exp(-1i*2*pi*(2*bn*d_s(m)/(lamda*Rc)+2*b_t*d_v(n)/(lamda*1000)+2*b_k*d_k(k)/(lamda*1000)));
...
for i=1:n_e
    ...
    J=J.';
    delta_phi_1=D*J;
    phi_real = angle(squeeze(PS_slc(:,PPSC_index(e(i,2))).*conj(PS_slc(:,PPSC_index(e(i,1))))));
...
G = G(:,SPS_set);
ref_point_index = find(SPS_set == find(PPSC_index == refer_point_index));
G(:,ref_point_index) = [];%参考点的选取需由前面搞定
...
for i=3:5
    ...
    X = [X(1:ref_point_index-1);0;X(ref_point_index:size(X,1))];
    SPS_1_list(1:size(SPS_1_list,1),i-1) = X;
end
...
for i=1:size(SPSC_index,1)
    ...
    if(min(dist)==Inf)
        ...
        y = squeeze(PS_slc(:,SPSC_index(i)).*conj(PS_slc(:,ref_index)));
        norm_y = norm(y,2);
        y = reshape(y,1,1,1,baseline_number);
        Gma = abs(sum(bsxfun(@times,M,y),4))/(sqrt(baseline_number)*norm_y);
%         for m = 1:size(d_s,2)
...
hold on
scatter3(x1(ref_point_index),y1(ref_point_index),vertical_h1(ref_point_index),20,'k','o');
hold on
